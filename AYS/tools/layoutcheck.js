...
(async () => {
  ...
  try{
    ...
    /* Bos ekran tasmaz; tasma VERIYLE gelir. Uzun metinler bilerek
       kirilmaz secildi: satiri tasiran sey genelde kirilamayan bir dizidir. */
    await page.evaluate(() => {
      const M = R.Model, U = R.U;
      for(let i = 0; i < 30; i++){
        R.S.cards.push({ id:'c' + i, front:'Cok uzun bir soru metni olabilir ' + i,
          back:'Bunun da epeyce uzun bir karsiligi var ' + i,
          stage:1, dueAt:U.todayISO(), history:[] });
        R.S.errors.push({ id:'er' + i, createdAt:new Date().toISOString(),
          tag:'K', topic:'Uzun bir konu adi olabilir ' + i,
          note:'Kok neden olarak yazilmis uzunca bir cumle ' + i });
      }
      for(let i = 0; i < 8; i++){
        R.S.exams.push({ id:'e' + i, date:U.iso(U.addDays(U.today(), -i * 3)),
          type:'Tam TYT', family:'TYT', kind:'full',
          publisher:'Cok uzun bir yayin adi ' + i, duration:165,
          tests:[{ name:'Turkce', correct:30, wrong:8, blank:2, minutes:null }],
