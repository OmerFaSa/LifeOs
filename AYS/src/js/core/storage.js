...
R.Storage = (function(){
  ...
  const BUDANABILIR = [
    ...
    { collection:'officeChats', label:'Ofis sohbet geçmişi',
      note:'Ajanlarla yapılmış yazışmalar. Kararlar ve teklifler ayrı '
  ...
  /* Kullanicinin girdigi icerik — ASLA budanmaz, yalnizca gosterilir.
     Bir sinav adayinin dokuz aylik kaydi, bu sistemin tasidigi en degerli
     seydir; yer acmak icin ona dokunulmaz. */
  const ICERIK = ['days', 'weeks', 'exams', 'errors', 'cards', 'reviews',
    'decisions', 'topics', 'protocols', 'videoNotes', 'activities', 'mood',
    'breaks', 'calendar', 'sessions', 'solved', 'sources', 'forecasts'];
  ...
  async function load(){
    const raw = await R.Store.get('storage');
  ...
  function breakdown(){
    const rows = R.Store.sizeByCollection ? R.Store.sizeByCollection() : [];
    const budaMap = {};
  ...
  async function prune(collections){
    ...
    const once = R.Store.localSize();
    for(const c of istenen){
      const rows = (await R.Store.list(c)) || [];
      for(const r of rows){
        if(r && r.id) await R.Store.remove(c + '/' + r.id);
      }
      /* Tek parcali kayitlar (usage gibi) dogrudan silinir. */
      await R.Store.remove(c);
    }
    const sonra = R.Store.localSize();

    /* Bellekteki kopyalar da temizlenir; yoksa ekran silinmis veriyi
       gostermeye devam eder. */
    if(istenen.indexOf('usage') >= 0){ S.usage = null; if(R.Friction) await R.Friction.load(); }
