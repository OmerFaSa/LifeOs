...
ESP.Screens.rutbe = (function(){
  const S = ESP.S, UI = ESP.UI, K = ESP.C;
  const { html, raw, when, map } = ESP.h;
  const L = function(){ return window.LIFEOS; };
  const XP = function(){ return ESP.XP; };
    ...
    return K.Card({ title:'Kademe atlayınca', sub:'Tercih · geri alınır',
      body:html`${K.Segmented({ act:'kutlama-sec', value:t, aria:'Kutlama biçimi',
        ...
        ${when(t === 'sakin' && V && V.sakinSeviye && k, () => html`<div class="rutbe-sakin-ornek mt-12">${raw(V.sakinSeviye({
          ust:'Örnek', ad:k.ad + ' ' + d.etiket, sistem:'esp', renk:k.id }).replace('vk vk-sakin', 'vk'))}</div>`)}
  ...
  function rozetTab(){
    ...
    const kd = (ESP.XP && ESP.XP.durum() || {}).kademeBilgi;
  ...
  function videolariBagla(){
    ...
    Array.prototype.forEach.call(hepsi, function(v){
      ...
      v.addEventListener('ended', function(){
        try{
          v.currentTime = ESP.Perde.donguNoktasi(geri, v.duration);
          var p = v.play();
  ...
  const handle = {
    ...
    async 'kutlama-sec'(el){
      ...
      ESP.UI.toast(el.dataset.value === 'sakin' ? 'Kademe atlama sakin parlamayla gösterilecek.' : 'Kademe atlama tam ekran perdeyle gösterilecek.',
        { undo:() => { P.kutlamaYaz(once); ESP.App.render(); } });
      ESP.App.render();
    },
