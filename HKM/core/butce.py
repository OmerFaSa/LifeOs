...
VARSAYILAN = {
    ...
    "usd_try": 0.0,             # elle girilir; 0 ise hesap yapilamaz
...
def month(con, cfg, date=None):
    ...
    oran = (toplam / tavan * 100.0) if tavan else None
...
def guard(con, cfg, date=None, cost_try=0.0):
    ...
    if not d["rate"]:
        return {"ok": False, "reason": "no-rate",
                "note": "USD/TRY kuru girilmemiş; TL hesabı yapılamaz."}
    dur = tavan * (float(d["stop_at_pct"] or 100) / 100.0)
    if d["spent_try"] + float(cost_try or 0) > dur:
        return {"ok": False, "reason": "ceiling",
                "spent": d["spent_try"], "limit": round(dur, 2),
                "note": "Aylık sınıra varıldı; ücretli çağrı durduruldu. "
                        "Kural motoru çalışmaya devam eder."}
    return {"ok": True, "spent": d["spent_try"], "limit": round(dur, 2)}
