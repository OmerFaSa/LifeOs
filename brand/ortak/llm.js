...
__NS__.LLM = (function(){
  ...
  const MODEL_STORE = '__DEPO__.llm.catalog';
  ...
  function keyOwner(value){
    const v = String(value || '').trim();
    if(!v) return null;
    const ids = Object.keys(__NS__.PROVIDERS);
    for(let i = 0; i < ids.length; i++){
      const p = __NS__.PROVIDERS[ids[i]];
  ...
  function keyProblem(providerId, value){
    const v = String(value || '').trim();
    const p = __NS__.PROVIDERS[providerId];
    if(!v || !p) return null;
    const owner = keyOwner(v);
    if(owner && owner !== providerId){
      return { level:'wrong', owner,
        text:'Bu anahtar ' + __NS__.PROVIDERS[owner].label + ' anahtarına benziyor, '
  ...
  function visionChain(cfg){
    ...
    __NS__.PROVIDER_ORDER.forEach(id => {
