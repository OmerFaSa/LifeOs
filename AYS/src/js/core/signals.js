...
R.Signals = (function(){
  ...
  async function save(sig){
    ...
    await R.Store.set('signals/' + sig.id, sig);
    return sig;
  }

  async function load(){
    S.signals = ((await R.Store.list('signals')) || []).map(norm);
    return S.signals;
  }
  ...
  async function sync(){
    ...
    for(const s of open()){
      ...
      if(s.kind === 'goodhart'){
        const cift = R.Goodhart.scan().filter(p => p.id === s.ref)[0];
    ...
    const bayraklar = R.Goodhart.flags()
    ...
    if(f.level === 'high' && (sonKapanis('friction') == null
      ...
      const sig = norm({ kind:'friction', ref:'friction',
        ...
        question:'Son iki haftada günde ' + f.window.perDay + ' dakika sistemi '
          + 'yönetmekle geçti. Hangi kaydı bırakmak çalışmanı bozmaz?',
