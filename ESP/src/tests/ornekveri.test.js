...
(function(){
  ...
  describe('172 Örnek veri — ESP', () => {
    ...
    it('oz-172 örnek profil HKM bağını alamaz (çıkışta gerçek profilin bağı kopmasın)', async () => {
      ...
      const M_ = 'esp';
      const eskiP = localStorage.getItem('esp.activeProfile'), eskiB = window.LIFEOS.HkmBag.sahip(M_);
      try{
        window.LIFEOS.HkmBag.birak(M_, eskiB || 'x');
        localStorage.setItem('esp.activeProfile', 'ornek');
        await ESP.Beacon.save({ enabled:true, token:'t', url:'http://127.0.0.1:4200' });
        expect(window.LIFEOS.HkmBag.sahip(M_)).toBe(null);
        expect(ESP.Beacon.settings().enabled).toBe(false);
      }finally{
        if(eskiP === null) localStorage.removeItem('esp.activeProfile'); else localStorage.setItem('esp.activeProfile', eskiP);
        window.LIFEOS.HkmBag.birak(M_, 'ornek');
        if(eskiB) window.LIFEOS.HkmBag.al(M_, eskiB);
        await ESP.Beacon.save({ enabled:false });
      }
    });
