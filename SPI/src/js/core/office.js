...
SP.Office = (function(){
  ...
  /* Metindeki sayilari cikarir (bkz. R.Office.numbersIn, AYS ile ayni
     bicim). Turkce yazimi tanir: "19,50" ondalik, "1.500" binlik, "%78" yuzde. */
  ...
  /* Brifingdeki butun sayilar. AYS/ESP'nin aksine SP.Office.brief() düz bir
     nesne döner (headline/data/findings katmanı yok, bkz. BRIEFS = {lab:
  ...
  /* Sayi sadakati: ajanin yazdigi her sayi brifingde var mi?
     Kucuk tam sayilar (≤12) ve yillar elenir — dogal dil sayilaridir. */
  function numberFidelity(text, brief){
    ...
    numbersIn(text).forEach(x => {
      const n = x.value;
      if(Number.isInteger(n) && n <= 12) return;
  ...
  /* Alan ihlali: ajan kendi masasinin disina cikti mi?
     ...
     ESP'nin iki-kosullu desenini kullanir (bkz. ESP.Office.validate):
     yalnizca BASKA bir ajanin anahtar kelimesi gecerse VE kendi anahtar
     kelimesi HIC gecmezse isaretlenir. Tek kosullu bir taboo listesi
     (AYS'nin kullandigi) burada guvenli degildir — orn. `lab` ajaninin
     kendi kirmizi bayrak metni "asiri antrenman" gibi baska bir alanin
     kelimesini gecerlikle icerebilir; iki kosul bu yanlis pozitifi onler. */
  function scopeBreaches(text, agent){
    if(!agent || !agent.keywords) return [];
    const s = String(text || '');
    const kendiGecti = agent.keywords.some(k => SP.trRe(k, 'i').test(s));
    if(kendiGecti) return [];
    const out = [];
    SP.AGENTS.forEach(other => {
      if(other.id === agent.id || other.id === 'patron' || !other.keywords) return;
      const eslesen = other.keywords.find(k => SP.trRe(k, 'i').test(s));
      if(eslesen) out.push(other.name + '\'in alanı (' + other.role + ') bu masaya ait değil');
    });
    return out;
  }
  ...
  /* ------------------------------------------------------------- denetim
     ...
     Uc sey aranir: ev kurallari (SP.GROUNDING.banned), alan ihlali
     (scopeBreaches) ve sayi sadakati (numberFidelity). Ilk ikisi
     `opts` olmadan da calisir; sayi sadakati yalnizca `opts.brief`
     verildiyse denetlenir. */
  function validate(text, opts){
    const o = opts || {};
    const s = String(text || '');
    const hits = SP.GROUNDING.banned.filter(b => b.re.test(s));
    const agent = o.agentId ? SP.AGENT_BY_ID[o.agentId] : null;
    const scope = scopeBreaches(s, agent);
    const unsupported = o.brief ? numberFidelity(s, o.brief) : [];

    const notes = hits.map(h => h.why).concat(scope);
    const note = notes.length
      ? notes.join(', ') + ' tespit edildi.'
      : (unsupported.length ? 'Brifingde geçmeyen sayı: ' + unsupported.join(', ') + '.' : '');

    return {
      ok:hits.length === 0 && scope.length === 0 && unsupported.length === 0,
