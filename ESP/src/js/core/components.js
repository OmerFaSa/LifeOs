/* Deklaratif bilesen seti.
   ...
   Not: eski ESP.UI yardimcilari bu setin ustune ince sarmalayici olarak durur;
   ekranlar kademeli olarak C.* API'sine gecirilir. */

window.ESP = window.ESP || {};

ESP.C = (function(){
  const { html, raw, when, cls, attrs, map } = ESP.h;
  const icon = name => raw(ESP.UI.icon(name));
  ...
  function Card(o){
    ...
    const head = (o.title || o.sub || o.badge || o.actions) ? html`
      <div class="card__head">
        <div>
          ${when(o.title, () => html`<h3>${o.title}${when(o.hint, () => raw(ESP.UI.hint(o.hint)))}</h3>`)}
  ...
  function Kutu(o){
    ...
    return html`<section class="${cls('kutu', o.class)}" data-oz="002" ${when(o.id, () => attrs({ id:o.id }))}>
      ${when(bas, () => html`<header class="kutu__bas">
        ...
        ${when(o.ad, () => html`<h2 class="kutu__ad">${o.ad}${when(o.ipucu, () => raw(ESP.UI.hint(o.ipucu)))}</h2>`)}
  ...
  /* SAYFA BÖLÜMLERİ (katalog 19, EKIP-PLANI T3): ekranın iç sekmeleri
     KARTLARA döner. Bütün bölümler alt alta çizilir; üstteki bölüm çubuğu
     sekme değil sayfa içi bağlantıdır — basınca o bölüme kayar. Sekme
  ...
  /* AYRINTI (D katmanı, EKIP-PLANI §1.1–1.2): 30 kelimeyi aşan açıklama
  ...
  function Ayrinti(o){
    return html`<div class="${cls('ayrinti', o.class)}">
      ...
      <details class="ayrinti__ac"><summary>${o.etiket || 'Neden?'}</summary>
        <div class="ayrinti__govde">${o.govde}</div></details>
    </div>`;
  }
  ...
  function bolumeGit(id){
    ...
    const h = el.querySelector('.sayfabolum__ad');
    ...
    return html`
      <section class="${cls('lrow', o.wide && 'lrow--wide', o.class)}"
        ...
        <div class="lrow__side">
          ${when(o.label, () => html`<div class="lrow__label">${o.label}${raw(ESP.UI.hint(o.hint || ''))}</div>`)}
  ...
  function Mic(o){
    if(!ESP.Voice || !ESP.Voice.supported()) return raw('');
    const on = ESP.Voice.isActive() && ESP.Voice.activeTarget() === o.target;
  ...
  function Drop(o){
    return html`<label class="drop" data-drop="${o.act}">
      ...
      <span class="drop__icon" aria-hidden="true">${raw(ESP.UI.icon(o.icon || 'upload'))}</span>
  ...
  function Empty(o){
    const sec = (ESP.App && ESP.App.sectionOf && ESP.S)
      ? ESP.App.sectionOf(ESP.S.route) : null;
    /* Motifi olmayan bölümde (yeni çekmeceler) boş bir kutu değil, ikon
       düşer: çizimsiz boş durum 010'u karşılamaz. */
    const motif = !o.icon && sec && ESP.UI.motif ? ESP.UI.motif(sec.id) : '';
    ...
    return html`
      <div class="${cls('nextup', o.calm && 'nextup--calm', o.sanat && 'nextup--sanat', !o.calm && 'h-canli')}">
        ...
        <div class="nextup__body">
          <div class="nextup__label">${o.label}${when(o.hint, () => raw(ESP.UI.hint(o.hint)))}</div>
  ...
  return {
    Kutu, ModulIsareti, SayfaBolumleri, bolumeGit, Ayrinti, SakinHata, TemaSecici, Card, Collapsible, Stat, Bar, Meter, Badge, Chip, Button, IconButton, Segmented, Subtabs,
    Entry, Ledger,
    PickCard, Toolbar,
