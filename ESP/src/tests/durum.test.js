...
(function(){
  ...
  describe('Boş durum sahnesi (010)', () => {
    it('çizim, tek cümle, tek eylem', () => {
      const d = coz(ESP.C.Empty({ text:'Henüz kayıt yok.',
        action:ESP.C.Button({ label:'İlk kaydı ekle', size:'sm', act:'go' }) }));
  ...
  describe('Sakin hata (011)', () => {
    it('ekran hatası: kırmızı yok, ilk cümle veri, tek düğme', () => {
      const d = coz(ESP.App.errorPanel(new Error('x is undefined')));
    ...
    it('teknik ileti silinmez, kapalı ayrıntıya iner', () => {
      const d = coz(ESP.App.errorPanel(new Error('<b>x</b> is undefined')));
      ...
      expect(coz(ESP.C.SakinHata({})).querySelectorAll('details').length).toBe(0);
    });
  });
})();
