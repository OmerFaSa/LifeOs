...
(function(){
  const { describe, it, expect, resetState, withTodayAsync } = __NS__.Test;
  const XP = __NS__.XP;
  ...
  describe('kazanma — tavan, kapsam ve gün penceresi', () => {
    ...
    it('GELECEĞE puan yazılmaz', async () => {
      await temiz();
      const e = tavansiz();
      const r = await XP.kazan(e.id, { gun:XP.gunKaydir(__NS__.U.todayISO(), 1) });
    ...
    it('pencereden eski güne puan yazılmaz — geriye dönük puan toplanmaz', async () => {
      await temiz();
      const e = tavansiz();
      const eski = XP.gunKaydir(__NS__.U.todayISO(), -(XP.GERI_GUN + 1));
  ...
  describe('defter kazanılanı yazar — geçmiş yeniden fiyatlanmaz', () => {
    ...
    it('her kayıt [adet, kazanılan XP] tutar', async () => {
      ...
      const kayit = XP._defter().gunler[__NS__.U.todayISO()][e.id];
    ...
    it('katalog fiyatı değişse de yazılmış gün DEĞİŞMEZ', async () => {
      ...
      const yazilan = XP.gunToplami(__NS__.U.todayISO());
      const gercekXP = e.xp;
      try{
        e.xp = gercekXP * 10;          /* katalogda fiyat on katına çıktı */
        expect(XP.gunToplami(__NS__.U.todayISO())).toBe(yazilan);
    ...
    it('katalogdan KALKMIŞ bir iş yabancı sayılmaz — geçmiş emek yok olmaz', async () => {
      ...
      const d = XP._defter();
      d.gunler[__NS__.U.todayISO()]['esp.eski.bir.is'] = [1, 33];
      d.toplam += 33;
      expect(XP.gunToplami(__NS__.U.todayISO())).toBe(e.xp + 33);
      const k = XP.kirilim();
  ...
  describe('eşitle — XP veriden türer, olaydan değil', () => {
    ...
    it('VERİLMEYEN kimliğe dokunmaz — kazan() ile girmiş satır silinmez', async () => {
      ...
      const satir = XP._defter().gunler[__NS__.U.todayISO()];
    ...
    it('pencereden eski güne eşitleme yapılmaz', async () => {
      await temiz();
      const e = tavanli();
      const eski = XP.gunKaydir(__NS__.U.todayISO(), -(XP.GERI_GUN + 2));
  ...
  describe('kutlama — kapalıyken atlanan seviye kaybolmaz', () => {
    ...
    it('bozuk defterdeki ileri «görülen» damgası kutlamayı susturmaz', async () => {
      ...
      await __NS__.Store.set(XP.YOL, { toplam:0, gorulen:18, gunler:{} });
      XP.bosalt();
      await XP.yukle();
