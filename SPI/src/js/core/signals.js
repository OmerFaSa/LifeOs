...
SP.Signals = (function(){
  ...
  async function save(sig){
    ...
    await SP.Store.set('signals/' + sig.id, sig);
    return sig;
  }

  async function load(){
    S.signals = ((await SP.Store.list('signals')) || []).map(norm);
    return S.signals;
  }
  ...
  async function sync(){
    ...
    for(const s of open()){
      ...
      if(s.kind === 'goodhart'){
        const cift = SP.Goodhart.scan().filter(p => p.id === s.ref)[0];
    ...
    const bayraklar = SP.Goodhart.flags()
    ...
    if(f.level === 'high' && (sonKapanis('friction') == null
      ...
      const sig = norm({ kind:'friction', ref:'friction',
        ...
        question:'Son iki haftada günde ' + f.window.perDay + ' dakika bu '
          + 'ekranlarda geçti. Hangi kaydı bırakmak sağlığını değil yalnızca '
          + 'tabloyu eksiltir?',
        snapshot:{ perDay:f.window.perDay } });
      await save(sig);
      return { changed:true, opened:sig };
    }
