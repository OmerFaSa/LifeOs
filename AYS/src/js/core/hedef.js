...
LIFEOS.Hedef = (function(){
  ...
  function kapasiteSenaryolari(h, paket, durum, bugun){
    ...
    /* TARİHİNİ KORU: «bu sürede olmaz» yalnız tarihi ötelemeyi önermez;
       kullanıcının KENDİ tarihine yetecek günlük vakit de bir seçenektir
       (5 dakikaya yuvarlanır, yukarı). Vakti zaten yetiyorsa gereksiz;
       günde 12 saati aşıyorsa gerçekçi bir seçenek değil — yazılmaz. */
    const gun = h.son_tarih ? gunFarki(bugun, h.son_tarih) : 0;
    if(gun > 0){
      const gerekliHaftalik = g.saat / (gun / 7) + (ek ? ek.saat : 0);
      const dk = Math.ceil(gerekliHaftalik * 60 / gunler / 5 - 1e-9) * 5;
      if(dk <= TARIH_KORU_UST_DK && !(kendi && dk <= kendi) && !out.some(x => x.kapasite.gunluk_dk === dk)){
        out.push({ ad:'Tarihini koru: günde ' + (dk % 60 === 0 ? (dk / 60) + ' saat' : dk + ' dakika')
            + (gunler < 7 ? ', haftada ' + gunler + ' gün' : ''),
          hiz:yuvarla(dk * gunler / 60, 2), birim:'saat/hafta', kapasite:{ gunluk_dk:dk },
          son_tarih:h.son_tarih, tarihKoru:true });
      }
    }
    return out;
  }
  const TARIH_KORU_UST_DK = 720;
