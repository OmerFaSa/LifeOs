/* 119 ÖNERİ GEÇMİŞİ — modül tarafı (ekip/T-DEVIR.md «H → T DEVRİ» 1).
   ...
   kararlar» (geçilen, geri alınan öneriler) o anda hiç görünmüyordu.
...
(function(){
  ...
  describe('119 Öneri geçmişi — SPİ Onaylar', () => {
    ...
    it('oz-119 bekleyen öneri varken geçmiş aynı işareti taşır', async () => {
      resetState();
      SP.S.ui.kingTeklifler = [{ id:7, konu:'Uyku analizi', durum:'bekliyor', secenekler:[{ id:'a', ad:'Kısa', metin:'Kısa özet' }] }];
      SP.S.proposals = [GECILEN];
      try{
        const k = dom(await SP.Screens.onaylar.render());
        expect(k.textContent.indexOf('Bekleyen öneri yok') < 0).toBe(true);
        expect(k.querySelector('[data-oz="119"] [data-oz="179"]')).toBeTruthy();
      }finally{ SP.S.ui.kingTeklifler = []; }
    });
  });
})();
