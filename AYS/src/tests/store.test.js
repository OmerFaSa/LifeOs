/* GERÇEK DEPO TESTLERİ — AYS.

   ------------------------------------------------------------------
   NEDEN BU DOSYA SONRADAN YAZILDI

   `node tools/kapsam.js AYS --ayrinti` ölçtü ve şunu yazdı:

       store.js  %46  12/26
         koşmayan: lGet, lSet, lDel, lList, init, get, set, remove,
                   list, clear, sizeByCollection, health …

   Yani depo API'sinin TAMAMI hiç koşmuyordu. Sebebi de görünür:
   öteki testler `R.Store`'u bellek içi sahte bir depoyla değiştirir
   (`mockStore`) ve bu DOĞRUDUR — onların konusu modelin mantığıdır.
   Ama sonuç şuydu: verinin gerçekten yazıldığı modül denenmiyordu.

   Bu deponun kendi kuralı ise en net kural: **kalıcılığa dokunmadan
   önce test yaz — verinin kaybolabileceği tek yer orası.** SPİ'de bu
   paket vardı (`store.test.js`, kapsam %85), AYS'de yoktu.

   Paket SPİ'ninkinden uyarlandı; iki `store.js` aynı yüzeyi açıyor
