...
ESP.App = (function(){
  ...
  ESP.Ornek = window.LIFEOS && LIFEOS.OrnekKip ? LIFEOS.OrnekKip.kur({ anahtar:'esp.activeProfile',
    varsayilan:'ben', silinecek:['esp.v1.ornek', 'esp.v1.ornek.oncesi'] }) : null;
  const ornekAcik = () => !!(ESP.Ornek && ESP.Ornek.acik());
  ...
  const globalHandle = {
    ...
    async 'ornek-gir'(){
      ...
      UI.confirmSheet('Örnek veriye geç', 'Örnek bir profil açılır. Gerçek verin ayrı anahtarda durur, '
        ...
        () => { if(ESP.Ornek.gir()) location.reload(); }, false, 'Örnek veriye geç');
    },
    async 'ornek-cik'(){
      if(!ESP.Ornek) return;
      UI.confirmSheet('Örnek veriden çık', 'Örnek kayıtlar silinir ve gerçek profiline dönersin.',
        () => { if(ESP.Ornek.cik()) location.reload(); }, false, 'Örnek veriden çık');
    },
  ...
  function rozetKutla(){
    ...
    if(sonuc && sonuc.sessiz){
      ...
      if(!(sonuc.sakin && V && V.sakinGoster && V.sakinGoster({ ust:'Yeni rozet', ad:r.ad, sistem:'ESP' })))
  ...
  function kutla(y){
    ...
    if(sonuc && sonuc.sessiz){
      ...
      if(!(sonuc.sakin && V && V.sakinGoster && V.sakinGoster({ ust:y.yeniKademe ? 'Yeni kademe' : 'Yeni rütbe',
        ad:ad + ' ' + y.etiket, sistem:'ESP', renk:renkAd })))
  ...
  async function boot(){
    try{
      ...
      if(window.LIFEOS && LIFEOS.HaftaOzet){
        ESP.HaftaOzet = LIFEOS.HaftaOzet.kur({ hkm:() => ESP.Beacon });
        const haftaTazele = () => ESP.HaftaOzet.cek().then(v => {
