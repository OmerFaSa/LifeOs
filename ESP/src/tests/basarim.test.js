...
(function(){
  const { describe, it, expect, withTodayAsync } = ESP.Test;
...
describe('Başarım — motor', () => {
  ...
  async function sifirla(){
    B().bosalt();
    try{ await ESP.Store.set('basarim', null); }catch(e){}
    await B().yukle();
  }
  ...
  it('yazılabilir pencere XP ile AYNI uzunluktadır', () => {
    ...
    expect(ESP.Basarim.PENCERE_GUN + 1).toBe(ESP.XP.pencere().length);
  });
...
describe('Başarım sayımı — veriden türetilir', () => {
  it('sayım üç alanı da döner', () => {
    const s = ESP.BasarimSayim.gunluk(ESP.U.todayISO());
  ...
  it('gün kümesi istenen günlerin hepsini döner', () => {
    const g = [ESP.U.todayISO()];
    const out = ESP.BasarimSayim.gunler(g);
  ...
  it('boş veride sayım sıfırdır, çökmez', () => {
    ...
    const eski = ESP.U.todayISO();
    const s = ESP.BasarimSayim.gunluk('1970-01-01');
