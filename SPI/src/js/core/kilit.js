/* ÜRETİLMİŞ KOPYA — BURAYI DÜZENLEME.
   Düzeltme brand/ortak/kilit.js içine yazılır; burası bir sonraki
...
/* GİZLİLİK KİLİDİ — katalog 176 (T5, ekip/EKIP-PLANI.md §4.2).
   ...
   KODU UNUTAN KİLİTLİ KALMAZ: «Kodu unuttum» bir dakika bekletir, sonra
...
window.LIFEOS.KILIT = (function(){
  ...
  function ac(modul, o){
    ...
    return new Promise(coz => {
      ...
      el.addEventListener('click', e => {
        ...
        b.disabled = true;
        const yaz = () => { not.textContent = 'Kilit ' + kalan + ' saniye sonra kalkacak; uygulama açılacak ve kilit yeniden kurulana kadar kapalı kalacak.'; };
        yaz();
        sayac = setInterval(() => {
          kalan--;
          if(kalan <= 0){ kaldir(modul); bitir('unutuldu'); return; }
          yaz();
        }, 1000);
      });
  ...
  function ayarHtml(modul, o){
    ...
    const govde = '<div class="kilit-ayar" data-kilit-ayar="' + m + '"' + (o.govde ? ' data-kilit-govde' : '') + '>'
      ...
      + '</div>'
      + '<p class="tiny dim mt-8">Kodu unutursan kilit ekranındaki «Kodu unuttum» bir dakika sonra kilidi kaldırır.</p>'
      + '</div>';
