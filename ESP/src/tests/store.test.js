...
/* GERÇEK DEPO TESTLERİ — verinin gerçekten yazıldığı katman.
   ...
   Öteki testler `ESP.Store`'u bellek içi sahte bir depoyla
...
(function(){
  const { describe, it, expect, realStore } = ESP.Test;
  const S = realStore;
  ...
  describe('Depo — yedek', function(){
    it('dışa aktarım künye ve veriyi birlikte taşır', async function(){
      ...
      expect(yedek.__meta.schemaVersion).toBe(ESP.SCHEMA_VERSION);
    ...
    it('geri yükleme bu cihazın HKM ayarını değiştirmez', async function(){
      await hkmIle(async function(){
        ...
        const yedekle = () => ({ __meta:{ app:uygulamaKimligi(), schemaVersion:ESP.SCHEMA_VERSION },
    ...
    it('geri yükleme eski kayıtları TAMAMEN değiştirir', async function(){
      ...
      const yedek = { __meta:{ app:uygulamaKimligi(), schemaVersion:ESP.SCHEMA_VERSION },
    ...
    it('daha yeni şemalı yedek reddedilir', function(){
      const r = S.readBackup({ __meta:{ schemaVersion:ESP.SCHEMA_VERSION + 1 }, data:{} });
    ...
    it('ice aktarma oncesi durumu saklar ve geri alinabilir', async function(){
      ...
      try{
        ...
        const oncekiTam = S.exportAll().data;
        await S.importAll({ __meta:{ app:uygulamaKimligi(), schemaVersion:ESP.SCHEMA_VERSION },
