/* MODEL KATMANI — saf kısımlar.
   ...
   ------------------------------------------------------------------
   NEDEN BU DOSYA SPİ'DEKİNİN NEREDEYSE AYNISI — VE BU ÖLÇÜLDÜ

   `node tools/kapsam.js ESP` şunu söyledi: `core/llm.js` %5,
   `core/quota.js` %14. SPİ'de aynı iki dosya sınanıyordu, ESP'de
   değil. Sebebi arandı ve bulundu — iki dosya ZATEN AYNI:

       llm.js     SPİ ↔ ESP : 737 satırın 735'i birebir
                  (fark: depo anahtarı `spi.llm.keys` / `esp.llm.keys`
                   ve iki başlık dizesi)
       quota.js   AYS ↔ ESP : 280 satırın 276'sı birebir

   Yani ESP'nin model katmanı sınanmamış değildi; SPİ'nin kopyası
   sınanıyordu ve kimse ikisinin aynı kaldığını denetlemiyordu. Bu
   testler o boşluğu kapatıyor ve aynı zamanda İLERİSİ İÇİN ÖNKOŞUL:
   iki kopyayı tek kaynağa indirmek (bkz. `tools/ortak.py`) ancak
   ikisinin de kapsamı varken güvenlidir — deponun kendi kuralı
   «önce kapsam, sonra bölme» ve birleştirme için de aynısı geçerli.

   ------------------------------------------------------------------
   NE SINANIR

   Ağ çağrısının kendisi burada denenmez (gerçek bir uç nokta ister),
   ama etrafındaki KARARLAR denenir ve asıl hata oraya saklanır:
     ...
     · kota — dakikalık hak dolunca BEKLE, günlük hak dolunca RET
