...
SP.Screens.rutbe = (function(){
    ...
    return K.Card({ title:'Kademe atlayınca', sub:'Tercih · geri alınır',
      body:html`${K.Segmented({ act:'kutlama-sec', value:t, aria:'Kutlama biçimi',
        ...
        ${when(t === 'sakin' && V && V.sakinSeviye && k, () => html`<div class="rutbe-sakin-ornek mt-12">${raw(V.sakinSeviye({
          ust:'Örnek', ad:k.ad + ' ' + d.etiket, sistem:'spi', renk:k.id }).replace('vk vk-sakin', 'vk'))}</div>`)}
  ...
  function videolariBagla(){
    ...
    Array.prototype.forEach.call(hepsi, function(v){
      ...
      v.addEventListener('ended', function(){
        try{
          v.currentTime = SP.Perde.donguNoktasi(geri, v.duration);
          var p = v.play();
  ...
  const handle = {
    ...
    async 'kutlama-sec'(el){
      ...
      SP.UI.toast(el.dataset.value === 'sakin' ? 'Kademe atlama sakin parlamayla gösterilecek.' : 'Kademe atlama tam ekran perdeyle gösterilecek.',
        { undo:() => { P.kutlamaYaz(once); SP.App.render(); } });
      SP.App.render();
    },
