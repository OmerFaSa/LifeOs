/* ÜRETİLMİŞ KOPYA — BURAYI DÜZENLEME.
   Düzeltme brand/seviye/basarim.js içine yazılır; burası bir sonraki
...
/* BAŞARIM MOTORU — rozet sayar, kazanımı yakalar, defteri tutar.
   ...
   üç arayüzün `src/js/core/` klasörüne kopyalanır. `ESP` ve `esp`
   ...
   okunur (`ESP.BasarimSayim`), özete EŞİTLENİR, rozetler eşiklerden
...
ESP.Basarim = (function(){
  var MOD = 'esp';
  var YOL = 'basarim';          /* depo anahtarı — `seviye`den AYRI */
  var PENCERE_GUN = 7;          /* geriye yazılabilir gün — XP ile aynı */

  /* BİR GÜNÜN UZUNLUĞU. «Bir günde en uzun odak» bundan büyük olamaz:
     daha büyük bir sayı bir ölçüm değil bir GİRİŞ HATASIDIR (48 yerine
     480 yazmak gibi) ve dışarı çıkarsa sonucu orantısız —

       `isaret()` HKM'ye `badge_focus_hours` gönderir;
       HKM sözleşmesi bu alanı 0–24 ile sınırlar (`sync_engine.py`,
           RANGES) ve sınır DOĞRUDUR, çünkü alan bir günün ölçüsüdür;
     ...
     TOPLAM DAKİKA KIRPILMAZ: orası bir gün değil günlerin toplamıdır
     ve hatalı bir giriş orada zaten şişmiş bir sayı olarak görünür. */
  var GUN_DAKIKA = 1440;
  ...
  function bosDefter(){
    return {
      ...
      /* BİR GÜNDE görülen en iyi değerler. Ay özetine sığmaz çünkü
         ...
         `odakTaban` budanan günlerden kalan tabandır. Gün kırılımı
         YAZILABİLİR PENCERE kadar tutulur (`PENCERE_GUN`, yedi gün);
         ...
         Yani düzeltme penceresi bir HAFTADIR: bir hafta içinde
         fark edilen bir yanlış düzelir, daha eskisi tabana yazılmış
         olur. Pencereyi büyütmek defteri büyütür; yedi gün, yanlış
         girilen bir günü fark etmeye yeten süre olarak seçildi
         (XP defterinin yazılabilir penceresiyle de aynı). */
  ...
  function normalize(ham){
    ...
    if(ham.enIyi && typeof ham.enIyi === 'object'){
      d.enIyi.odakDakika = gunluk(sayi(ham.enIyi.odakDakika));
  ...
  function gocTabani(d){
    ...
    Object.keys(d.gunler).forEach(function(g){
      var dk = gunluk((d.gunler[g] || {}).dakika);
      if(dk > en) en = dk;
    });
    /* Eski defterdeki rekor da gün uzunluğuyla sınırlanır: sınırsızken
       yazılmış bir değer göçerken düzelir. */
    return en >= gunluk(d.enIyi.odakDakika) ? 0 : gunluk(d.enIyi.odakDakika);
  }
  ...
  /* Ekrana çıkan rekoru yeniden hesaplar: penceredeki günlerin en
     ...
     Pencerede en çok sekiz gün durur (`PENCERE_GUN` + bugün), yani bu
     gezinme ucuzdur ve her yazmada bir kez koşar. */
  function odagiTazele(d){
    var en = gunluk(d.enIyi.odakTaban);
    Object.keys(d.gunler).forEach(function(g){
      var dk = gunluk((d.gunler[g] || {}).dakika);
      if(dk > en) en = dk;
    });
  ...
  function odakHtml(dakika){
    ...
    if(!o) return '';
    var kac = (ESP.h && ESP.h.esc) ? ESP.h.esc : function(x){ return x; };
