...
(function(){
  const { describe, it, expect, withTodayAsync } = R.Test;
...
describe('Başarım — motor', () => {
  ...
  async function sifirla(){
    B().bosalt();
    try{ await R.Store.set('basarim', null); }catch(e){}
    await B().yukle();
  }
...
describe('Başarım sayımı — veriden türetilir', () => {
  it('sayım üç alanı da döner', () => {
    const s = R.BasarimSayim.gunluk(R.U.todayISO());
  ...
  it('gün kümesi istenen günlerin hepsini döner', () => {
    const g = [R.U.todayISO()];
    const out = R.BasarimSayim.gunler(g);
  ...
  it('boş veride sayım sıfırdır, çökmez', () => {
    ...
    const s = R.BasarimSayim.gunluk('1970-01-01');
