...
ESP.LLM = (function(){
  ...
  const MODEL_STORE = 'esp.llm.catalog';
  ...
  function keyOwner(value){
    const v = String(value || '').trim();
    if(!v) return null;
    const ids = Object.keys(ESP.PROVIDERS);
    for(let i = 0; i < ids.length; i++){
      const p = ESP.PROVIDERS[ids[i]];
  ...
  function keyProblem(providerId, value){
    const v = String(value || '').trim();
    const p = ESP.PROVIDERS[providerId];
    if(!v || !p) return null;
    const owner = keyOwner(v);
    if(owner && owner !== providerId){
      return { level:'wrong', owner,
        text:'Bu anahtar ' + ESP.PROVIDERS[owner].label + ' anahtarına benziyor, '
  ...
  function supportsVision(cfg){
    const p = ESP.PROVIDERS[cfg && cfg.provider];
    if(!p) return false;
