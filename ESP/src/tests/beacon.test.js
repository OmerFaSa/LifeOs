...
(function(){
  ...
  describe('HKM işareti — sözleşme', () => {
    ...
    it('gövde yalnızca sayı taşır, içerik taşımaz', () => {
      resetState();
      const p = B().payload(BUGUN);
      expect(Object.keys(p.metrics).sort().join(',')).toBe('level_step,level_sub,level_tier,practice_minutes,retention,retention_cards,synthesis_gap_days,xp_today,xp_total');
