...
R.UI = (function(){
  ...
  function confirmSheet(title, message, onConfirm, danger, onay){
    sheet({
      ...
      footer:String(R.h.html`${R.C.Button({ label:'Vazgeç', act:'sheet-close' })}
        ${R.C.Button({ label:onay || 'Evet, devam et', tone:danger ? 'danger' : 'primary', act:'confirm-yes' })}`),
      noFocus:true,
    });
  ...
  /* AJAN PORTRESI — `.agentav` kutusunun icine biner.
     ...
     dosya adi ajanin kimliginden turer — `ajan-ays-patron.webp`.
  ...
  function pp(id){
    ...
    if(!t) return '';
    return R.h.raw('<img class="agentav__pp" src="img/marka/ajan-ays-'
  ...
  function onayMuhru(){
    ...
    var d = (R.XP && R.XP.durum) ? R.XP.durum() : null;
  ...
  function ppKare(id){
    ...
    var t = String(id).replace(/[^a-z0-9_-]/gi, '');
    if(!t) return '';
    return R.h.raw('<img class="agentkare__pp" src="img/marka/ajan-kare-ays-'
      + t + '.webp" alt="" aria-hidden="true" loading="lazy"'
      + ' onerror="this.remove()">');
  }
