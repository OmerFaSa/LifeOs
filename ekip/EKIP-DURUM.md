...
- **T'DE (kullanıcı kararları 2026-09-25 18:10; K bunlara dokunmasın):** ses/video çözümlemesi **Gemini** ile + belge (PDF/Word/metin) yerel okuma · **112** çakışma kartı **dilim düzeyinde** (saat eklenmez; aynı gün aynı dilim) · **127** kapsam seçimi **hedef değişikliklerine** (bugün küçük, hafta/kalıcı orta) · eski Codex dalı **arşiv dalıyla silinir** (`arsiv/gpt-lifeos-materyal-merkezi`). ✅ portreler 888c763 (çevreden doldurma; K'nin beyaz zemini duvarda leke bırakıyordu).
  ...
  - ✅ **127 kapsam seçimi — hedef değişikliklerinde** — AYS günlük süre önerisinin kartında «Yalnız bugün / Bu hafta / Kalıcı» (`ONERI.kapsamHtml`). Seçim öneriyi dönüştürür, uygulamaz: bugün/hafta → `gecici-sure` (bugün–bugün / bugünden haftanın son gününe), kalıcı → `gunluk-sure`; yeniden doğrulanır, onay ayrı (`R.Proposals.kapsamla`). Seviye kataloğun ve paylaşılan kuralın (kalıcı = büyük) — kullanıcıya «kalıcı orta» demiştim; kural değişmedi, rapor edildi. SPİ/ESP'de geçici karşılığı olan hedef aksiyonu yok; kart yalnız AYS'de. Test `kapsamkart.test.js` (AYS, 2).
  - ✅ **Canlı deneme (2026-09-25 gece):** HKM 127.0.0.1 + üç arayüz eşleşti; `entegre.js` temiz; yeni özellikler tarayıcıda tıklanarak 24/24 (120 pazar 18:00'e sabitlenmiş saatle, 112, 127, 119, 167, 172 giriş/çıkış, HKM PDF/ses/video). İki gerçek hata bulundu ve düzeltildi (bu commit): **(a)** örnek kipte eşleşme yapılırsa HKM bağı «ornek»e geçip çıkışta gerçek profili koparıyordu → örnek profil bağ almaz; **(b)** 127'de kart «orta», seçili «Kalıcı» «büyük» diyordu → ortak kart seçili kapsamın seviyesini söyler (`kapsamId`).
  - 🔴 **H'ye bulgu (mevcut test, T dokunmadı):** `ESP/src/tests/hedefler.test.js:88` · `HD().ozetler()` gerçek bugünü kullanıyor (test `BUGUN`'ü sabitlemiyor) · 2026-09-26 (İstanbul) itibarıyla `plan.ilerleme.durum` «geride» döner, test «yolunda» bekler · temiz HEAD'de de kırmızı (değişiklik kenara alınarak doğrulandı); CI UTC gece yarısından sonra kırmızıya döner. Öneri: testi `withToday(BUGUN, …)` içine al.
